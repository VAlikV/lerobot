...
class GeneralRobotEnv(gym.Env):
    ...
    def __init__(
        ...
        self._state_features = self.robot.observation_features - self._image_keys
        self._action_features = self.robot.action_features
        self._action_dim = len(self._action_features)
    ...
    def _get_observation(self) -> RobotObservation:
        ...
        state = {key: obs_dict[key] for key in self._state_features}
        images = {key: obs_dict[key] for key in self._image_keys}

        return {"agent_pos": state, "pixels": images}
    ...
    def _setup_spaces(self) -> None:
        ...
        if current_observation is not None:
            ...
            observation_spaces[OBS_STATE] = gym.spaces.Box(
                low=self.limits["obs_low"],
                high=self.limits["obs_high"],
        ...
        bounds = {}
        bounds["min"] = -np.ones(self._action_dim)*self.limits["action_low"]
        bounds["max"] = np.ones(self._action_dim)*self.limits["action_high"]

        if self.use_gripper:
            bounds["min"] = np.concatenate([bounds["min"], [self.limits["gripper_low"]]])
            bounds["max"] = np.concatenate([bounds["max"], [self.limits["gripper_high"]]])
    ...
    def step(self, action) -> tuple[RobotObservation, float, bool, bool, dict[str, Any]]:
        ...
        robot_action = {key: action[i] for i, key in enumerate(self._action_features)}
