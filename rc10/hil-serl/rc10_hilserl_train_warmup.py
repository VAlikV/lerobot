...
def run_pretraining(policy, offline_buffer, optimizers):
    print("[Learner] Starting offline pretraining...")

    for step in range(PRETRAIN_STEPS):
        if len(offline_buffer) < PRETRAIN_BATCH_SIZE:
            continue

        batch = offline_buffer.sample(PRETRAIN_BATCH_SIZE)
        ...
        if step % 500 == 0:
            print(f"[Pretrain] Step {step}/{PRETRAIN_STEPS}, "
...
def run_learner(
    ...
    if PRETRAIN_DATASET_REPO_ID is not None:
        warmap_dataset = LeRobotDataset(repo_id=PRETRAIN_DATASET_REPO_ID, download_videos=False)
        warmap_buffer = ReplayBuffer.from_lerobot_dataset(
                lerobot_dataset=warmap_dataset, device=DEVICE,
                state_keys=state_keys, storage_device="cpu",
                capacity=OFFLINE_BUFFER_CAPACITY)
        
        if len(warmap_buffer) > 0:
            run_pretraining(policy, warmap_buffer, optimizers)

            try:
                state_dict = {k: v.detach().cpu().numpy() for k, v in policy.state_dict().items()}
                parameters_queue.put_nowait(state_dict)
                print("[Learner] Sent pretrained weights to actor")
            except Full:
                pass
